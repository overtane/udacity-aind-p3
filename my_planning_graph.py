...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for a in self.a_levels[level-1]:
            for s in a.effnodes:
                self.s_levels[level].add(PgNode_s(s.symbol, s.is_pos))
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check if preconditions have competing needs
        for e1 in node_a1.prenodes:
            for e2 in node_a2.prenodes:
                if e1.symbol == e2.symbol:
                    if e1.is_pos != e2.is_pos:
        ...
        # check if parents have competing needs
    ...
    def h_levelsum(self) -> int:
        ...
        For each goal in the problem, determine the level cost and add them together
